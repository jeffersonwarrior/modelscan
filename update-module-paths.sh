...
if [ -f "go.mod" ]; then
    ...
    sed -i 's|github.com/nexora/modelscan|github.com/jeffersonwarrior/modelscan|g' go.mod
fi
...
for dir in sdk/*/; do
    if [ -f "${dir}go.mod" ]; then
        ...
        sed -i 's|github.com/nexora/modelscan|github.com/jeffersonwarrior/modelscan|g' "${dir}go.mod"
    fi
done
...
if [ -f "sdk/go.mod" ]; then
    ...
    sed -i 's|github.com/nexora/modelscan|github.com/jeffersonwarrior/modelscan|g' sdk/go.mod
fi

# Update sdk.go imports
if [ -f "sdk/sdk.go" ]; then
    echo "  ✓ Updating sdk/sdk.go"
    sed -i 's|github.com/nexora/modelscan|github.com/jeffersonwarrior/modelscan|g' sdk/sdk.go
fi
...
if [ -f "main.go" ]; then
    ...
    sed -i 's|github.com/nexora/modelscan|github.com/jeffersonwarrior/modelscan|g' main.go
fi
...
for file in providers/*.go; do
    if [ -f "$file" ]; then
        echo "  ✓ Updating $file"
        sed -i 's|github.com/nexora/modelscan|github.com/jeffersonwarrior/modelscan|g' "$file"
    fi
done

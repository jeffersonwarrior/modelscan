package parsers

import (
	"testing"

	"github.com/jeffersonwarrior/modelscan/internal/tooling"
)

func TestXAIParser_Parse(t *testing.T) {
	parser := &XAIParser{}

	// Example xAI XML response
	response := `Here is my response with a tool call:
<tool_call>
<id>call_xai_123</id>
<name>get_weather</name>
<arguments>{"location":"San Francisco","unit":"fahrenheit"}</arguments>
</tool_call>
That's what I need.`

	toolCalls, err := parser.Parse(response)
	if err != nil {
		t.Fatalf("Failed to parse xAI response: %v", err)
	}

	if len(toolCalls) != 1 {
		t.Fatalf("Expected 1 tool call, got %d", len(toolCalls))
	}

	tc := toolCalls[0]
	if tc.ID != "call_xai_123" {
		t.Errorf("Expected ID call_xai_123, got %s", tc.ID)
	}
	if tc.Name != "get_weather" {
		t.Errorf("Expected Name get_weather, got %s", tc.Name)
	}
	if tc.Type != "function" {
		t.Errorf("Expected Type function, got %s", tc.Type)
	}

	// Check arguments
	location, ok := tc.Args["location"].(string)
	if !ok || location != "San Francisco" {
		t.Errorf("Expected location 'San Francisco', got %v", tc.Args["location"])
	}

	unit, ok := tc.Args["unit"].(string)
	if !ok || unit != "fahrenheit" {
		t.Errorf("Expected unit 'fahrenheit', got %v", tc.Args["unit"])
	}
}

func TestXAIParser_ParseMultiple(t *testing.T) {
	parser := &XAIParser{}

	// Multiple tool calls
	response := `I'll use these tools:
<tool_call>
<id>call_1</id>
<name>get_weather</name>
<arguments>{"location":"NYC"}</arguments>
</tool_call>
And also:
<tool_call>
<id>call_2</id>
<name>get_time</name>
<arguments>{"timezone":"EST"}</arguments>
</tool_call>
Done.`

	toolCalls, err := parser.Parse(response)
	if err != nil {
		t.Fatalf("Failed to parse multiple tool calls: %v", err)
	}

	if len(toolCalls) != 2 {
		t.Fatalf("Expected 2 tool calls, got %d", len(toolCalls))
	}

	// Verify first tool call
	if toolCalls[0].Name != "get_weather" {
		t.Errorf("Expected first tool name get_weather, got %s", toolCalls[0].Name)
	}
	if toolCalls[0].ID != "call_1" {
		t.Errorf("Expected first ID call_1, got %s", toolCalls[0].ID)
	}

	// Verify second tool call
	if toolCalls[1].Name != "get_time" {
		t.Errorf("Expected second tool name get_time, got %s", toolCalls[1].Name)
	}
	if toolCalls[1].ID != "call_2" {
		t.Errorf("Expected second ID call_2, got %s", toolCalls[1].ID)
	}
}

func TestXAIParser_ParseEmpty(t *testing.T) {
	parser := &XAIParser{}

	// Response with no tool calls
	response := `Here is my text response without any tool calls.`

	toolCalls, err := parser.Parse(response)
	if err != nil {
		t.Fatalf("Failed to parse text-only response: %v", err)
	}

	if len(toolCalls) != 0 {
		t.Errorf("Expected 0 tool calls for text response, got %d", len(toolCalls))
	}
}

func TestXAIParser_ParseCompact(t *testing.T) {
	parser := &XAIParser{}

	// Compact XML without whitespace
	response := `<tool_call><id>compact_id</id><name>test_func</name><arguments>{"key":"value"}</arguments></tool_call>`

	toolCalls, err := parser.Parse(response)
	if err != nil {
		t.Fatalf("Failed to parse compact XML: %v", err)
	}

	if len(toolCalls) != 1 {
		t.Fatalf("Expected 1 tool call, got %d", len(toolCalls))
	}

	if toolCalls[0].Name != "test_func" {
		t.Errorf("Expected name test_func, got %s", toolCalls[0].Name)
	}
}

func TestXAIParser_ParseInvalidJSON(t *testing.T) {
	parser := &XAIParser{}

	// Tool call with invalid JSON arguments
	response := `<tool_call>
<id>call_1</id>
<name>test</name>
<arguments>not valid json</arguments>
</tool_call>`

	_, err := parser.Parse(response)
	if err == nil {
		t.Error("Expected error for invalid JSON arguments")
	}
}

func TestXAIParser_ParseMalformed(t *testing.T) {
	parser := &XAIParser{}

	// Malformed XML (missing closing tag)
	response := `<tool_call>
<id>call_1</id>
<name>test</name>
<arguments>{"key":"value"}</arguments>`

	toolCalls, err := parser.Parse(response)
	if err != nil {
		t.Fatalf("Should not error on malformed XML, just return empty: %v", err)
	}

	if len(toolCalls) != 0 {
		t.Errorf("Expected 0 tool calls for malformed XML, got %d", len(toolCalls))
	}
}

func TestXAIParser_Format(t *testing.T) {
	parser := &XAIParser{}

	if parser.Format() != tooling.FormatXAIXML {
		t.Errorf("Expected format %s, got %s", tooling.FormatXAIXML, parser.Format())
	}
}

func TestXAIParser_ProviderID(t *testing.T) {
	parser := &XAIParser{}

	if parser.ProviderID() != "xai" {
		t.Errorf("Expected provider ID xai, got %s", parser.ProviderID())
	}
}

func TestXAIParser_Capabilities(t *testing.T) {
	parser := &XAIParser{}

	caps := parser.Capabilities()

	if !caps.SupportsToolCalling {
		t.Error("Expected SupportsToolCalling to be true")
	}
	if !caps.SupportsParallel {
		t.Error("Expected SupportsParallel to be true")
	}
	if caps.SupportsStreaming {
		t.Error("Expected SupportsStreaming to be false for XML format")
	}
	if caps.Format != tooling.FormatXAIXML {
		t.Errorf("Expected format %s, got %s", tooling.FormatXAIXML, caps.Format)
	}
}

func TestXAIParser_Registration(t *testing.T) {
	// Parser should auto-register via init()
	parser, err := tooling.GetParser("xai")
	if err != nil {
		t.Fatalf("Failed to get registered xAI parser: %v", err)
	}

	if parser.ProviderID() != "xai" {
		t.Errorf("Expected xai parser, got %s", parser.ProviderID())
	}
}

...
cd /home/agent/modelscan/internal/http
...
# 3. Coverage check (95% threshold)
echo "3. Coverage check (95% threshold)..."
go test -coverprofile=c.out . >/dev/null 2>&1
COVERAGE=$(go tool cover -func=c.out | grep total | awk '{print $3}' | sed 's/%//')

# Use bc for floating point comparison
if (( $(echo "$COVERAGE < 95.0" | bc -l) )); then
    echo "❌ FAIL: Coverage ${COVERAGE}% < 95.0%"
    echo "   Spec requires exactly 95%+ coverage"
    exit 1
fi
echo "✅ PASS: Coverage ${COVERAGE}% >= 95.0%"
echo
